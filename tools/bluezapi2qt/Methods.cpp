...
void Methods::parse(const QString& line)
{
    ...
    if (rx.indexIn(line) != -1) {
        ...
        m_currentMethod->m_outParameters = list.at(1).toLower().split(", ", QString::SkipEmptyParts);
        m_currentMethod->m_name = list.at(2);
        m_currentMethod->m_inParameters = list.at(3).split(", ", QString::SkipEmptyParts);
        m_currentMethod->m_stringTags = list.at(4).toLower().split(", ", QString::SkipEmptyParts);
